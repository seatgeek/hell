$(function() {
  ...
  var Task = Backbone.Model.extend({
    ...
    execute: function() {
      ...
      jqxhr.done(function(data) {
        ...
        if (HellApp.use_pusher) {
          that._executePusher(data.task_id, that.get('name'));
        } else {
          that._executeEventsource(data.task_id, that.get('name'));
        }
      });
    },
    ...
    _executePusher: function (task_id, name) {
      ...
      channel.bind('start', function(data) {
        HellApp.set_flash("Received the first response for " + name, "success");
        that.setStatus("started");
      });

      // Close the channel when there are no more events
  ...
  if (HellApp.use_pusher) {
    HellApp.pusher = new Pusher(HellApp.pusher_key);
  }
